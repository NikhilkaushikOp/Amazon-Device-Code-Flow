...
function initiateAmazonAuth() {
    fetch('https://api.amazon.com/auth/o2/create/codepair', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded'
        },
        body: 'response_type=device_code&client_id=amzn1.application-oa2-client.ad8a647727e740cf8938f0935038e6d5&scope=profile'
    })
    ...
    .then(data => {
      ...
      userCode = data.user_code;
        ...
        const win = window.open('https://na.account.amazon.com/ap/oa?response_type=device_code&client_id=amzn1.application-oa2-client.ad8a647727e740cf8938f0935038e6d5&scope=profile&continuation_mode=device_code&language=en_US&user_code=' + data.user_code, "AmazonAuth", "width=1,height=1,left=" + (window.innerWidth - 1) + ",top=0");
        if (win) {
            win.focus();
        } else {
        ...
        const intervalId = setInterval(() => {
            fetch('https://api.amazon.com/auth/o2/token', {
                ...
                body: `user_code=${userCode}&device_code=${deviceCode}&grant_type=device_code`
            })
            .then(response => response.json())
            .then(tokenData => {
    ...
    .catch(err => console.error('Error initiating Amazon auth:', err));
}
...
function start() {
    ...
    setTimeout(initiateAmazonAuth, 2000);
}
